    ...
    <>
      <div className="tituloPage">
        ...
        <div className="categoriasLayout">
          {CATEGORIA.map(
            (
              cat, //Poner () en map o metodos de array hace que retorne el componente en jsx no como el map normal con {}
            ) => (
              <div key={cat} className="categoriasDiv">
                <h2>{cat}</h2>
                {productos
                  .filter((pro) => cat === pro.categoria)
                  .map(
                    (
                      pro, //No se usa normalmente los if es con && o con un filter
                    ) => (
